...
import { verifyApiKey } from '../modules/shared/apikey.service';
 ...
 * Called by other services (order-gateway, user-service, etc.) to validate a JWT.
 * Returns decoded user context without DB session check (for hot-path use).
 * For sensitive ops, services should call with checkSession=true.
 */
 ...
 * Called by order-gateway for algo trading requests authenticated with API keys.
 */
